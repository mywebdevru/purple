...
<header class="header" id="site-header">
	...
	<div class="header-content-wrapper">
        ...
        <div class="control-block">
            @auth
                @component('user.components.header.control_block_item',['count' => $data->friendship_requests_count])
                    @slot('icon')
                        <svg class="olymp-happy-face-icon"><use xlink:href="{{ asset('svg-icons/sprites/icons.svg#olymp-happy-face-icon') }}"></use></svg>
                        <div class="label-avatar bg-blue">{{ $data->friendship_requests_count }}</div>
                    @endslot
                    @slot('title')
                        Они хотят дружить
                    @endslot
                    @slot('notification')
                        @each('user.components.header.friendship_request', $data->friendshipRequests, 'request')
                    @endslot
                    @slot('button')
                        ...
                        <form id="accept-all-friends" action="{{ route('friend.store') }}" method="POST" style="display: none;">
                            @csrf
                            <input type="hidden" name="requested_friendship" value="{{ $data->friendshipRequests }}">
                        </form>
                    @endslot
                @endcomponent
                ...
                @component('user.components.header.page_owner', ['full_name' => $data->full_name, 'creed' => $data->creed]) @endcomponent
            @else
